...
public class DatingLayout_ extends ViewGroup implements View.OnClickListener {
  // Data
  public  ImageView     mFaceView;     // оцениваемая фотография
  public  InfoView_      mInfoView;     // информация о пользователе
  public  ResourcesView_ mResView;     // ресурсы
  public  StarsView_     mStarsView;    // контрол со звездами для оценки
  public  InformerView_  mInformerView; // контрол с всплывающей панелей и 2 кнопками(чат,профиль)
  ...
  //---------------------------------------------------------------------------
  public DatingLayout_(Context context) {
    super(context);
    ...
    // Info
    mInfoView = new InfoView_(context);
    addView(mInfoView);

    // Informer, profile, chat
    mInformerView = new InformerView_(context);
    addView(mInformerView);
    
    mResView = new ResourcesView_(context);
    addView(mResView);
    ...
    */
    ...
    // Stars
    mStarsView = new StarsView_(context,mInformerView);
    addView(mStarsView);
  ...
  public void onProfileBtnClick() {
    ((DatingGallery_)getParent()).onProfileBtnClick();
  }
  //---------------------------------------------------------------------------
  public void onChatBtnClick() {
    ((DatingGallery_)getParent()).onChatBtnClick();
  }
  //---------------------------------------------------------------------------
  public void onRate(int index) {
    ((DatingGallery_)getParent()).onRate(index);
  }
  //---------------------------------------------------------------------------
