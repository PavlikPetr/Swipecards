...
abstract public class PluralRules 
{
    ...
    static int attrForQuantity(int quantity) {
    ...
    static String stringForQuantity(int quantity) {
        switch (quantity) {
            case QUANTITY_ZERO:
                return "zero";
            case QUANTITY_ONE:
    ...
    static PluralRules ruleForLocale(Locale locale) {
        
    	return allRules.get(locale.getLanguage());
    }

    static boolean hasLocale(Locale locale) {
        return allRules.containsKey(locale.getLanguage());
    }
}
