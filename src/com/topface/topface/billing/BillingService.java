...
public class BillingService extends Service implements ServiceConnection {
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        ...
        try {
            ...
            purchases = null;
            Log.e(TAG, "Verify purchase exception" + e.getMessage());
            e.printStackTrace();
        }

        //Если проверка покупки не прошла (например не верный сертификат), то не пытаемся проверять ее в дальнейшем
        if (purchases == null) {
            return;
        }
        ...
        for (VerifiedPurchase vp : purchases) {
            ...
            ResponseHandler.purchaseResponse(this, vp.purchaseState, vp.productId,
                    vp.orderId, vp.purchaseTime, vp.developerPayload, signedData, signature);
        }
