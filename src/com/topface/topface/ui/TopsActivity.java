...
public class TopsActivity extends TrackedActivity {
    ...
    private void showCitiesDialog() {
        ...
        //Добавляем первым элементом город пользователя
        final City ownCity = new City();
        ownCity.id = CacheProfile.getProfile().city_id;
        ownCity.full = CacheProfile.getProfile().city_full;
        ownCity.name = CacheProfile.getProfile().city_name;
        ...
        builder.setSingleChoiceItems(cities, selectedItem, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int position) {
                City city = position == 0 ? ownCity : Data.cityList.get(position);
