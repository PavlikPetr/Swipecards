...
class GlideTransformationFactory(val mContext: Context) {
    fun construct(@GlideTransformationType.GlideTransformation type: Long, radiusOnline: Float?, outSideLine: Float?, circleColor: Int? = Color.WHITE): Array<Transformation<Bitmap>> =
            when (type) {
