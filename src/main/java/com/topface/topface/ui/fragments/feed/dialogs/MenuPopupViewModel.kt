 ...
 */
abstract class MenuPopupViewModel(user: FeedUser) {

    val userPhoto = ObservableField(user.photo)
    val type = ObservableField(if (user.online) GlideTransformationType.ONLINE_CIRCLE_TYPE else GlideTransformationType.CROP_CIRCLE_TYPE)
