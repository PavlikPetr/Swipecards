...
public class GridLayout extends com.topface.topface.ui.gridlayout.ViewGroup {
    ...
    private static final int COLUMN_AUTO_FIT = R.styleable.GridLayout_columnsAutoFit;
    private static final int COLUMN_WIDTH = R.styleable.GridLayout_columnWidth;
    public static final int DEF_COLUMN_WIDTH = 95;
    ...
    public GridLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            setColumnAutoFit(a.getBoolean(COLUMN_AUTO_FIT, DEFAULT_AUTO_FIT));
            setColumnWidth(a.getInt(COLUMN_WIDTH, DEF_COLUMN_WIDTH));
        } finally {
            a.recycle();
        }
    }
    ...
    private void setColumnAutoFit(boolean autoFit) {
        mAutoFit = autoFit;
    }
...
//    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0, N = getChildCount(); i < N; i++) {
            ...
            int cx = !isLayoutRtl(this) ? paddingLeft + leftMargin + dx :
    ...
    */
    ...
    private static Alignment createSwitchingAlignment(final Alignment ltr, final Alignment rtl) {
        return new Alignment() {
            @Override
            int getGravityOffset(View view, int cellDelta) {
                return (!isLayoutRtl(view) ? ltr : rtl).getGravityOffset(view, cellDelta);
            }

            @Override
            public int getAlignmentValue(View view, int viewSize) {
                return (!isLayoutRtl(view) ? ltr : rtl).getAlignmentValue(view, viewSize);
            }
        };
    }
